        ...
        for( int t=1; t<=tasks; t++ )
   		 {
   			...
   			//positionAndSkip(bpmnModel, lastUserTask);
   			 ...
   			 // adiciona uma decisão antes da tarefa 1
			 if(t==1){
   			 		String gid = "xgw_" + t;
					...
					process.addFlowElement(new SequenceFlow(lastUserTask.getId(), gid));
					edges.add(new SequenceFlow(lastUserTask.getId(), gid));
   		...
   		//	SequenceFlow pass = new SequenceFlow( gate.getId(), lastUserTask.getId() );
   		//	pass.setName("yes");
   		//	pass.setConditionExpression("${'yes' == 'yes'}");
   		//	SequenceFlow fail = new SequenceFlow( gate.getId(), fin.getId() );
   		//	fail.setName("no");
   		//	fail.setConditionExpression("${'yes' != 'yes'}");
			...
			throws Exception
	{
		...
		for( int fg : empId )
			...
			{	
				...
				File file = new File("flowable-wind11"+ fg +".bpmn");
